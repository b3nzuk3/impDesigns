...
export default function QuotePage() {
  ...
  const getWhatsAppUrl = () => {
    ...
    if (productType === 'tape') {
      ...
      msg += `- Thickness: ${metalThickness}%0A`;
      msg += `- Mounting: ${metalMounting === '3m-adhesive' ? '3M 468MP High-Bond Adhesive' : 'Dual 3.2mm Rivet Holes'}%0A`;
      msg += `- Barcode: ${barcodeType === 'code128' ? 'Code 128 / Code 39' : '2D DataMatrix / QR'}%0A`;
      msg += `- Volume: ${metalQuantity} Plates%0A`;
      msg += `- Est. Rate: KES ${aluminiumEst.unitPrice}/plate (Total: KES ${aluminiumEst.total.toLocaleString()})%0A`;
    }

    msg += `%0APlease review artwork and dispatch timeline.`;
    return `https://wa.me/254722404647?text=${msg}`;
  };
